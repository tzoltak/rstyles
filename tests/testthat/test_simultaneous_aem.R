...
mSml <- mirt(resp,
             ...
             method = "QMCEM", CTOL = 0.1, verbose = FALSE)
...
test_that("Item parameters of simultaneous A, E, M RS (with 5-point scale) recovers in estimation with reasonable MSEs.", {
  ...
  expect_lt(mean((slopes[, 4] - estItemPars[, 4])^2), 0.12)
  expect_lt(mean((intercepts[, 1] - estItemPars[, 26])^2), 0.04)
  expect_lt(mean((intercepts[, 2] - estItemPars[, 27])^2), 0.04)
  expect_lt(mean((intercepts[, 3] - estItemPars[, 28])^2), 0.05)
  expect_lt(mean((intercepts[, 4] - estItemPars[, 29])^2), 0.04)
})
